...
using static FaddeevaSharp.Faddeeva;
using System;

namespace FaddeevaSharpTest
{
    [TestClass]
    public class FaddeevaSharpTest
    {
        ...
        private static double RelErr(double a, double b)
        {
            ...
            {
                if ((double.IsNaN(a) && !double.IsNaN(b)) || (!double.IsNaN(a) && double.IsNaN(b)) ||
                    (double.IsInfinity(a) && !double.IsInfinity(b)) || (!double.IsInfinity(a) && double.IsInfinity(b)) ||
                    (double.IsInfinity(a) && double.IsInfinity(b) && ((a * b) < 0)))
        ...
        private static Complex C(double re, double im) => new(re, im);

        private static void TST(string name, int NTST, Complex[] z, Complex[] w, Func<double,double> re_fun, Func<Complex, double, Complex> comp_fun, double isc)
        {
            Console.WriteLine("Function: {0}", name);
            double errmax = 0d;
            for(int i = 0; i < NTST; ++i)
            {
                var fw = comp_fun(z[i], 0d);
                double re_err = RelErr(w[i].Real, fw.Real);
                double im_err = RelErr(w[i].Imaginary, fw.Imaginary);

                Console.WriteLine("{5}({0}) = {1} (vs. {2}), re/im rel. err. = {3} / {4})", z[i], fw, w[i], re_err, im_err, name);

                Assert.IsTrue(re_err <= 1e-13);
                Assert.IsTrue(im_err <= 1e-13);

                if (re_err > errmax) errmax = re_err;
                if (im_err > errmax) errmax = im_err;
            }

            Assert.IsTrue(errmax <= 1e-13);
            Console.WriteLine("Max relative error = {0}", errmax);

            Console.WriteLine("Checking {0}(x) for special case...", name);
            for(int i = 0; i < 5000; ++i) // TODO: increase no of loops to 10000
            {
                double x = Pow(10d, -300d + i * 600d / (10000 - 1));
                
                double re_result = re_fun(x);
                var comp_result = comp_fun(new(x, x * isc), 0d);
                Console.WriteLine("{0}({1}) = ({2}) / ({3})", name, x, re_result, comp_result);
                double re_err = RelErr(re_result, comp_result.Real);

                Assert.IsTrue(re_err <= 1e-13);
                if (re_err > errmax) errmax = re_err;

                re_result = re_fun(-x);
                comp_result = comp_fun(new(-x, x * isc), 0d);
                Console.WriteLine("{0}({1}) = ({2}) / ({3})", name, -x, re_result, comp_result);
                re_err = RelErr(re_result, comp_result.Real);
            ...
            }
            {
                ...
                double re_err = RelErr(re_fun(Inf), comp_fun(new(Inf, 0d), 0d).Real);

                Assert.IsTrue(re_err <= 1e-13);
                if (re_err > errmax) errmax = re_err;

                Console.WriteLine("-Inf");
                re_err = RelErr(re_fun(-Inf), comp_fun(new(-Inf, 0d), 0d).Real);

                Assert.IsTrue(re_err <= 1e-13);
                if (re_err > errmax) errmax = re_err;

                Console.WriteLine("NaN");
                re_err = RelErr(re_fun(NaN), comp_fun(new(NaN, 0d), 0d).Real);

                Assert.IsTrue(re_err <= 1e-13);
                if (re_err > errmax) errmax = re_err;
            }
            Assert.IsTrue(errmax <= 1e-13);
            System.Console.WriteLine("Max relative error = {0}", errmax);
        }

        [TestMethod]
        public void WComplex()
        {
            const int NTST = 57;
            Complex[] z = new Complex[NTST]
            {
                C(624.2,-0.26123),
                C(-0.4,3d),
                C(0.6,2d),
            ...
            Complex[] w = new Complex[NTST]
            {
                ...
                C(-5.586035480670854326218608431294778077663867e-161,
                5.586035480670854326218608431294778077663867e-160),
                /*C(NaN,
                NaN),*/
            ...
            for(int i = 0; i < NTST; ++i)
            {
                ...
                System.Console.WriteLine("w({0}) = {1} (vs. {2}), re/im rel. err. = {3} / {4})", z[i], fw, w[i], re_err, im_err);
        ...
        public void ErfComplex()
        {
            ...
            TST("Erf", NTST, z, w, Erf, Erf, 1e-20);
        }
        [TestMethod]
        public void ErfiComplex()
        {
            const int NTST = 1;
            Complex[] z = new Complex[NTST]
            {
            ...
            Complex[] w = new Complex[NTST] { // erfi(z[i]), computed with Maple
                C(1.081032284405373149432716643834106923212,
                1.926775520840916645838949402886591180834)
            };

            TST("Erfi", NTST, z, w, Erfi, Erfi, 0);
        }
        [TestMethod]
        public void ErfcxComplex()
        {
            const int NTST = 1;
            Complex[] z = new Complex[NTST]
            {
            ...
            Complex[] w = new Complex[NTST] { // erfcx(z[i]), computed with Maple
                C(0.3382187479799972294747793561190487832579,
                -0.1116077470811648467464927471872945833154)
            };

            TST("Erfcx", NTST, z, w, Erfcx, Erfcx, 0);
        }
        [TestMethod]
        public void ErfcComplex()
        {
            const int NTST = 30;
            Complex[] z = new Complex[NTST]
            {
            ...
            Complex[] w = new Complex[NTST] { // erfc(z[i]), evaluated with Maple
            ...
            TST("Erfc", NTST, z, w, Erfc, Erfc, 1e-20);
        }

        [TestMethod]
        public void DawsonComplex()
        {
            const int NTST = 48;
            Complex[] z = new Complex[NTST]
            {
            ...
            TST("Dawson", NTST, z, w, Dawson, Dawson, 1e-20);
        }
    }
}