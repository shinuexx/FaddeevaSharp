...
namespace FaddeevaSharp
{
    /// <summary>
    /// Provides Faddeeva functions implementation. Details can be found at <see href="http://ab-initio.mit.edu/wiki/index.php/Faddeeva_Package">Faddeeva Package</see>
    /// </summary>
    public static class Faddeeva
    {
        private const double Inf = double.PositiveInfinity;
        private const double HugeVal = double.PositiveInfinity;
        private const double NaN = double.NaN;
        private const double Epsilon = 2.2204460492503131e-016;
        
        private static Complex Exp(Complex value)
        {
            if (value.Real == double.NegativeInfinity && (double.IsNaN(value.Imaginary) || value.Imaginary == double.PositiveInfinity))
                return new(0, 0);
            else
                return Complex.Exp(value);
        }
        private static double Exp(double x) => Math.Exp(x);

        /// <summary>
        /// Calculate <see href="http://en.wikipedia.org/wiki/Faddeeva_function">Faddeeva function</see>, scaled <see cref="Complex"/> complementary <see href="https://en.wikipedia.org/wiki/Error_function">error function</see>
        /// <code>w(z) = exp(-z^2) erfc(-iz)</code>
        /// </summary>
        /// <param name="z">A <see cref="Complex"/> number</param>
        /// <param name="relerr">A max relative error</param>
        /// <returns>The result of Faddeeva function</returns>
        public static Complex W(Complex z, double relerr = 0d)
        {
            if (z.Real == 0d)
                return new(Erfcx(z.Imaginary),
                    z.Real); // give correct sign of 0 in cimag(w)
            else if (z.Imaginary == 0d)
                return new(Exp(-Sqr(z.Real)), Wim(z.Real));
                 ...
                 && (ya > 0.1 || (x > 8 && ya > 1e-10) || x > 28)))
            {
                ...
                { // nu <= 2
                    ...
                    { // nu == 2, w(z) = i/sqrt(pi) * z / (z*z - 0.5)
                        ...
                        ret = new(denom * (xs * di - ya * dr), denom * (xs * dr + ya * di));
                    }
                }
                else
                ...
                if (y < 0)
                {
                    ...
                    Complex tmp = new((ya - xs) * (xs + ya), 2 * xs * y);
                    tmp = Exp(tmp);
                    return 2.0 * tmp - ret;
                }
                else
    ...
    below.  Therefore, we use x < 10 here. */
            else if (x < 10)
            {
                ...
                else
                {
                    ...
                    ret = new(coef1 * cos2xy + coef2 * sinxy * Sinc(xs * y, sinxy),
        ...
        finish:
            return ret + new Complex((0.5 * c) * y * (sum2 + sum3),
                                     (0.5 * c) * CopySign(sum5 - sum4, z.Real));
        }

        /// <summary>
        /// Calculate <see href="http://en.wikipedia.org/wiki/Faddeeva_function">Faddeeva function</see> for special-case <c><see cref="W(Complex, double)">W</see>(<paramref name="x"/>).<see cref="Complex.Imaginary">Imaginary</see></c> of real <paramref name="x"/>
        /// </summary>
        /// <param name="x">A <see cref="double"/> number</param>
        /// <returns>The result of Faddeeva function</returns>
        public static double Wim(double x)
        {
            if (x >= 0)
            {
                if (x > 45)
        ...
        /// <param name="z">A <see cref="Complex"/> number</param>
        /// <param name="relerr">A max relative error</param>
        /// <returns><c>erfcx(z) = exp(z^2) erfc(z)</c></returns>
        public static Complex Erfcx(Complex z, double relerr = 0d)
        {
            return W(new(-z.Imaginary, z.Real), relerr);
        }
        /// <summary>
        /// The scaled complementary error function
        /// </summary>
        /// <param name="x">A <see cref="double"/> number</param>
        /// <returns><c>erfcx(x) = exp(x^2) erfc(x)</c></returns>
        public static double Erfcx(double x)
        {
            if (x >= 0)
            {
                ...
                return x < -26.7 ? HugeVal : (x < -6.1 ? 2 * Exp(x * x)
                                               : 2 * Exp(x * x) - ErfcxY100(400 / (4 - x)));
        }

        /// <summary>
        /// The <see href="https://en.wikipedia.org/wiki/Error_function">error function</see>
        /// </summary>
        /// <param name="z">A <see cref="Complex"/> number</param>
        /// <param name="relerr">A max relative error</param>
        /// <returns>The error function of complex arguments</returns>
        public static Complex Erf(Complex z, double relerr = 0d)
        {
            double x = z.Real, y = z.Imaginary;
            if (y == 0d)
                return new(Erf(x), y /* preserve sign of 0 */);
            if (x == 0d) // handle separately for speed & handling of y = Inf or NaN
                return new(x, // preserve sign of 0
     ...
     problems from multiplying exponentially large and small quantities. */
            if (x >= 0)
            {
                ...
                return 1.0 - Exp(mRe_z2) *
                  (new Complex(Cos(mIm_z2), Sin(mIm_z2))
                   * W(new(-y, x), relerr));
            }
            else
            { // x < 0
                ...
                { // duplicate from above to avoid fabs(x) call
                    ...
                    return new(NaN, y == 0 ? 0 : NaN);
                /* don't use complex exp function, since that will produce spurious NaN
                   values when multiplying w in an overflow situation. */
                return Exp(mRe_z2) *
                  (new Complex(Cos(mIm_z2), Sin(mIm_z2))
                   * W(new(y, -x), relerr)) - 1.0;
            }
        ...
        taylor:
            {
                Complex mz2 = new(mRe_z2, mIm_z2); // -z^2
...
*/
        ...
        /// </summary>
        /// <param name="x">A <see cref="double"/> number</param>
        /// <returns>The error function of double arguments</returns>
        public static double Erf(double x)
        {
            ...
            if(x >= 0)
            {
                ...
                return 1d - Exp(mx2) * Erfcx(x);
            }
            else
            {
                if (x > -8e-2) goto taylor;
                return Exp(mx2) * Erfcx(-x) - 1d;
            }
        ...
        /// <param name="z">A <see cref="Complex"/> number</param>
        /// <param name="relerr">A max relative error</param>
        /// <returns><c>erfi(z) = -i * erf(iz)</c></returns>
        public static Complex Erfi(Complex z, double relerr = 0d)
        {
            Complex e = Erf(new(-z.Imaginary, z.Real), relerr);
            return new(e.Imaginary, -e.Real);
        }
        ...
        /// </summary>
        /// <param name="x">A <see cref="double"/> number</param>
        /// <returns><c>erfi(z) = -i * erf(ix)</c></returns>
        public static double Erfi(double x)
        {
            return x * x > 720 ? (x > 0 ? Inf : -Inf) : Exp(x * x) * Wim(x);
        }
        ...
        /// <param name="z">A <see cref="Complex"/> number</param>
        /// <param name="relerr">A max relative error</param>
        /// <returns><c>erfc(z) = 1 - erf(z)</c></returns>
        public static Complex Erfc(Complex z, double relerr = 0d)
        {
            double x = z.Real, y = z.Imaginary;

            if (x == 0d)
                return new(1d, /* handle y -> Inf limit manually, since
                exp(y^2) -> Inf but Im[w(y)] -> 0, so
                IEEE will give us a NaN when it should be Inf */
                    y * y > 720 ? (y > 0 ? -Inf : Inf)
                    : -Exp(y * y) * Wim(y));
            if(y == 0d)
            {
                if (x * x > 750) // underflow
                    return new(x >= 0 ? 0d : 2d,
                        -y); // preserve sign of 0
                return new(x >= 0 ? Exp(-x * x) * Erfcx(x) : 2d - Exp(-x * x) * Erfcx(-x),
                    -y);// preserve sign of 0
            }
            ...
            if (x >= 0)
                return Exp(new Complex(mRe_z2, mIm_z2))
                  * W(new(-y, x), relerr);
            else
                return 2.0 - Exp(new Complex(mRe_z2, mIm_z2))
                  * W(new(y, -x), relerr);
        }
        ...
        /// </summary>
        /// <param name="x">A <see cref="double"/> number</param>
        /// <returns><c>erfc(x) = 1 - erf(x)</c></returns>
        public static double Erfc(double x)
        {
            if (x * x > 750)
                return x >= 0d ? 0d : 2d;
            return x >= 0d ? Exp(-x * x) * Erfcx(x) : 2d - Exp(-x * x) * Erfcx(-x);
        }

        /// <summary>
        /// The <see href="http://en.wikipedia.org/wiki/Dawson_function">Dawson function</see>
        /// </summary>
        /// <param name="z">A <see cref="Complex"/> number</param>
        /// <param name="relerr">A max relative error</param>
        /// <returns><c>Dawson(z) = sqrt(pi)/2  *  exp(-z^2) * erfi(z)</c></returns>
        public static Complex Dawson(Complex z, double relerr = 0d)
        {
            const double spi2 = 0.8862269254527580136490837416705725913990;
            double x = z.Real, y = z.Imaginary;

            // handle axes separately for speed & proper handling of x or y = Inf or NaN
            if (y == 0)
                return new(spi2 * Wim(x),
                         -y); // preserve sign of 0
            if (x == 0)
            {
                double y2 = y * y;
            ...
            if (y >= 0)
            {
                if (y < 5e-3)
                {
                    if (Abs(x) < 5e-3)
                        goto taylor;
                    else if (Abs(mIm_z2) < 5e-3)
                        goto taylor_realaxis;
                }
                Complex res = Exp(mz2) - W(z, relerr);
                return spi2 * new Complex(-res.Imaginary, res.Real);
            }
            else
            { // y < 0
                if (y > -5e-3)
                { // duplicate from above to avoid fabs(x) call
                    ...
                    return new(x == 0 ? 0 : NaN, NaN);
                var res = W(-z, relerr) - Exp(mz2);
                return spi2 * new Complex(-res.Imaginary, res.Real);
            }
        ...
        */
        ...
        /// </summary>
        /// <param name="x">A <see cref="double"/> number</param>
        /// <returns><c>Dawson(x) = sqrt(pi)/2  *  exp(-x^2) * erfi(x)</c></returns>
        ...
        private static double Sinc(double x, double sinx) => Abs(x) < 1e-4 ? 1d - (0.1666666666666666666667) * x * x : sinx / x;
        private static double SinhTaylor(double x) => x * (1 + (x * x) * (0.1666666666666666666667 + 0.00833333333333333333333 * (x * x)));
        ...
        private static double ErfcxY100(double y100)
        {
            switch ((int)y100)
            {
                case 0:
        ...
        private static double WImY100(double y100, double x)
        {
            switch ((int)y100)
            {
                case 0:
